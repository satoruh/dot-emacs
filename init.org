...
#+begin_src emacs-lisp
  (custom-set-variables
   ...
   '(tramp-persistency-file-name (my/user-data "tramp"))
   ...
   ;; org-roam
   '(org-roam-db-location (my/user-data "org-roam.db"))
...
#+begin_src emacs-lisp
  (use-package asdf
    ...
    (asdf-binary (or (executable-find "asdf") "asdf")))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-o") 'toggle-input-method) ;orig. open-line
...
#+begin_src emacs-lisp
  ...
  (defvar my/disable-display-line-numbers-modes
    '(vterm-mode
      term-mode
      vc-annotate-mode
      completion-list-mode
  ...
  (mapc
   (lambda (mode)
     (add-hook (intern (concat (symbol-name mode) "-hook"))
               #'my/disable-display-line-numbers))
   my/disable-display-line-numbers-modes)
#+end_src
...
vterm-moduleのビルドにcmakeとlibtool（libtoolize）が必要。libvtermは
emacs-libvterm側がパッチ済みのものを取得してビルドするため、システムの
libvterm-devは不要。依存は =make deps= でまとめて導入する。
#+begin_src shell
  make deps
#+end_src
...
~/.config と user-emacs-directory 以下の org ファイルは保存時に自動でタングルする。生成物は Makefile と同じく read-only にする。シンボリックリンク経由で開いた場合も判定できるよう、開いたままのパスと truename の両方で比較する。

#+begin_src emacs-lisp
  (defvar my/org-babel-auto-tangle-directories
    (list "~/.config/" user-emacs-directory))

  (defun my/org-babel-auto-tangle ()
    (when (seq-some (lambda (dir)
                      (or (string-prefix-p (expand-file-name dir) buffer-file-name)
                          (string-prefix-p (file-truename dir)
                                           (file-truename buffer-file-name))))
                    my/org-babel-auto-tangle-directories)
      (dolist (file (org-babel-tangle))
        (set-file-modes file #o444))))

  (add-hook 'org-mode-hook
            (lambda ()
              (add-hook 'after-save-hook #'my/org-babel-auto-tangle nil t)))
#+end_src
